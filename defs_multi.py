...
def clear_chat_history():
    st.session_state.messages = [{"role": "ai", "content": "어떤 식당을 찾으시나요?"}]
...
def print_messages():    
    ...
    if 'messages' in st.session_state and len(st.session_state['messages']) > 0:
        ...
        for chat_message in st.session_state['messages']:
            st.chat_message(chat_message.role).write(chat_message.message)  
...
###############################[체크표시 필터링]##################################
def search_restaurants(question, local_jeju_city=False, local_seogwipo_city=False):
    filtered_results = []
...
def extract_keywords_from_text(question):
    ...
    prompt = keywords_template.format(question=question)
    response = model.generate_content(prompt, generation_config={'response_mime_type':'application/json'})
...
def sorted_df(question, df):
  ...
  json_data = extract_keywords_from_text(question) # keywords_json
...
def search_chain(question, llm):
    ...
    return chain.invoke({"query_string": query_string, "store":store} )
...
def extract_recommendation_keywords_from_text(question):
    ...
    # 프롬프트 생성
    prompt = recommendation_keywords_template.format(question=question)
...
def filter_chroma_db(question, chroma_store):
    ...
    filtered_result = search_restaurants(question)
    # 질의를 통해 JSON 데이터 추출
...
# LLM을 이용한 최종 응답 생성
def recommendation_chain(question, llm, history, recommendation_store):
    # 데이터베이스에서 관련 정보 검색
    ...
    prompt = recommendation_chat_template.format()
    chain = prompt | llm | StrOutputParser()
    # response = llm.generate_content(recommendation_chat_template)
    # generated_text = response.text.strip().replace("**", "").replace("\n", "").replace("  ", " ")

    return chain.invoke({'question':question, 'db_info':db_info})
...
def main(question, user):
    ...
    if classification['Classification'] == '검색형':
        print('분류 >> 검색형')
        answer = search_chain(question, model)

    elif classification['Classification'] == '추천형':
        print('분류 >> 추천형')
        answer = recommendation_chain(question, model, history, recommendation_store)

    else:
        print('분류 >> 기타')
        answer = other_chain(question, model)
