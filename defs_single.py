...
def filter_chroma_db(query, local_jeju_city, local_seogwipo_city, chroma_store):
    ...
    if filters:
        # 필터링 실행
        filtered_items = \
        [item for item in metadata['metadatas'] if all(item.get(key) == value for key, value in filters.items())]
        ...
        if not filtered_items and 'name' in extracted_data:
            ...
            # 유사한 항목의 메타데이터 추출
            filtered_items = \
            [item for item in metadata['metadatas'] if item['name'] in [x[0] for x in similar_items]]
    ...
    if filtered_items:
        ...
        filtered_items = \
        [filtered_items[i] for i in range(len(filtered_items)) if summaries[i] in [x[0] for x in similar_summaries]]

    # 최종적으로 address 필터링 적용
    if sidebar_filter:
        filtered_items = \
        [item for item in filtered_items if item.get('address') == sidebar_filter['address']]
...
def recommendation_chain(query,local_jeju_city, local_seogwipo_city, llm, recommendation_store):
    ...
    **2. 카페 더 콘테나**  \n
    ...
    여러 음식점을 답변해야 하는 경우, 위의 예시1)에 따라 질문에 맞는 음식점을 깔끔하게 정리하여 답변해 주세요.
...
def other_chain(question, llm):
    ...
    prompt = other_prompt.format(question=question)
    response = llm.generate_content(prompt)
...
def main(query, local_jeju_city, local_seogwipo_city, df):
    ...
    if classification['Classification'] == '검색형':
        # print('분류 >> 검색형')
        return classification['Classification'], search_main(query, df)

    elif classification['Classification'] == '추천형':
        # print('분류 >> 추천형')
        return classification['Classification'], recommendation_main(query, local_jeju_city, local_seogwipo_city, recommendation_store)

    else:
        # print('분류 >> 기타')
        return '기타', other_main(query)
